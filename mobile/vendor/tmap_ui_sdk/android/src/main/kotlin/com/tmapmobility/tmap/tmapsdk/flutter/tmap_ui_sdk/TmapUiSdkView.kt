...
class TmapUiSdkView(
  ...
  private var routeRequestRetryCount = 0
  private var routeRequestScheduled = false
  ...
  override fun getView(): View {
    synchronized(this) {
      ...
      if (isFlutterDisplaying && !routeRequested) {
        requestRouteWhenReady()
      }
      if (false && isFlutterDisplaying && !routeRequested) {
        routeRequested = true
  ...
  private fun requestRouteWhenReady() {
    if (routeRequested || navigationRequestModel == null) {
      return
    }

    if (fragmentContainer.width <= 0 ||
      fragmentContainer.height <= 0 ||
      routeRequestRetryCount < MIN_ROUTE_RETRY_BEFORE_REQUEST) {
      retryRouteRequest("waiting for platform view to settle")
      return
    }
      ...
      navigationFragment.requestRoute(
        ...
        object : TmapUISDK.RouteRequestListener {
          ...
          override fun onFail(errorCode: Int, errorMsg: String?) {
            val errorMessage = "error: $errorCode Msg:${errorMsg ?: "NA"}"
            Log.e(TAG,"requestRoute - onFail() $errorCode $errorMsg")
            if (errorCode == -7 && routeRequestRetryCount < MAX_ROUTE_RETRY_COUNT) {
              routeRequested = false
              retryRouteRequest(errorMsg ?: "route request failed before activity was ready")
              return
            }
            navigationFragment.activity?.let {
              it.runOnUiThread {
  ...
  private fun retryRouteRequest(reason: String) {
    if (routeRequestScheduled) {
      return
    }
    if (routeRequestRetryCount >= MAX_ROUTE_RETRY_COUNT) {
      Log.e(TAG, "requestRoute retry exhausted: $reason")
      SDKStatusStreamer.success(TmapSDKStatusMsgModel(TmapSDKStatus.DISMISS_REQ, "-7"))
      return
    }

    routeRequestRetryCount += 1
    routeRequestScheduled = true
    Log.d(TAG, "requestRoute delayed ($routeRequestRetryCount): $reason")
    mainHandler.postDelayed({
      routeRequestScheduled = false
      requestRouteWhenReady()
  ...
  companion object {
    private const val MIN_ROUTE_RETRY_BEFORE_REQUEST = 4
    private const val MAX_ROUTE_RETRY_COUNT = 80
