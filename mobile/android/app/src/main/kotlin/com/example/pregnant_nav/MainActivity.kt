package com.example.pregnant_nav

import com.baseflow.geolocator.GeolocatorPlugin
import com.eyedeadevelopment.fluttertts.FlutterTtsPlugin
import dev.fluttercommunity.plus.sensors.SensorsPlugin
import io.flutter.embedding.android.FlutterFragmentActivity
import io.flutter.embedding.engine.FlutterEngine
import io.flutter.plugins.GeneratedPluginRegistrant

class MainActivity : FlutterFragmentActivity() {
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        GeneratedPluginRegistrant.registerWith(flutterEngine)
        if (!flutterEngine.plugins.has(GeolocatorPlugin::class.java)) {
            flutterEngine.plugins.add(GeolocatorPlugin())
        }
        if (!flutterEngine.plugins.has(FlutterTtsPlugin::class.java)) {
            flutterEngine.plugins.add(FlutterTtsPlugin())
        }
        if (!flutterEngine.plugins.has(SensorsPlugin::class.java)) {
            flutterEngine.plugins.add(SensorsPlugin())
        }
    }
}
